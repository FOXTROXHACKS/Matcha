...
-- ================================================================= --
-- PEGA ESTO JUSTO DEBAJO DE LA ÚLTIMA LÍNEA DE TU SCRIPT            --
-- (Debajo de: _G.TweenService = TweenService)                       --
-- ================================================================= --

local player = game:GetService("Players").LocalPlayer
local character = player.Character or player.CharacterAdded:Wait()

-- Intentamos usar getRoot() de tu ejecutor de forma segura, sino usamos el clásico
local root
pcall(function() root = getRoot() end)
if not root then root = character:WaitForChild("HumanoidRootPart") end

local spawnablesFolder = workspace:FindFirstChild("DebrisClient")

if spawnablesFolder then
    local monedas = spawnablesFolder:GetChildren()
    
    for i, v in pairs(monedas) do
        local touchPart = v:FindFirstChild("HitDetect")
        
        if touchPart and touchPart:IsA("Part") then
            -- Extraemos la posición exacta a la que queremos ir
            local endPos = touchPart.Position
            local startPos = root.Position
            
            -- Calculamos la distancia y el tiempo para que la velocidad sea constante
            local distancia = (startPos - endPos).Magnitude
            local velocidad = 100 -- Cambia a 80 o 100 si lo quieres más rápido
            local tiempoDeViaje = distancia / velocidad
            
            if tiempoDeViaje < 0.1 then tiempoDeViaje = 0.1 end

            -- Usamos el TweenInfo nativo del script que enviaste
            local info = _G.TweenService.TweenInfo.new(tiempoDeViaje, "Linear", "Out")
            
            -- Creamos el tween directo a la Posición
            local tween = _G.TweenService:Create(root, info, { Position = endPos })
            tween:Play()
            
            -- CRÍTICO: Usamos 'wait' (el que sobreescribió tu script) en lugar de 'task.wait'
            -- Esto obliga al TweenService personalizado a procesar los cuadros por segundo
            wait(tiempoDeViaje)
        end
    end
else
    warn("No se encontró la carpeta DebrisClient")
end
